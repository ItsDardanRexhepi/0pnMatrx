...
|---|---|---|
| Read access to the private security package | deploy CI / image build | Install `morpheus_security` into the same environment as this platform so the seam binds real enforcement. How that access is granted is kept with the private deployment, not published here. Absent → platform runs with `SECURITY_BACKEND=noop` (the Morpheus gate blocks nothing; the public checks still run, including the per-agent tool boundary, the seam-level refusals, the Rexhepi (URF) gate, the Glasswing audit block and fail-closed owner/OTP). |
