        ...
        for cls in [n for n in ast.walk(tree) if isinstance(n, ast.ClassDef)] + [tree]:
            for node in getattr(cls, "body", []):
                if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    owner = cls.name if isinstance(cls, ast.ClassDef) else ""
                    yield str(path.relative_to(_BLOCKCHAIN)), owner, node
...
def test_a_raw_receipt_wait_lives_only_in_the_shared_wait():
    """THE LOAD-BEARING CENSUS. Fails on the tree before this fix, naming the
    34 functions that waited inline."""
    inline = {
...
def test_every_sender_waits_through_the_shared_wait():
    ...
    unaccounted = {
        key for path, owner, node in _functions_outside_services()
        for key in [(path, owner, node.name)]
        if key in _senders() and key not in primitive
